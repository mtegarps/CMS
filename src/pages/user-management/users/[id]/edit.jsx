...
const Page = ({ roles, companies, user }) => {
  ...
  const submitHandler = async (e) => {
    ...
    await axios
      .put(`${process.env.NEXT_PUBLIC_API_URL}/user/update`, formData, {
        headers: {
                ...
                <div className="relative z-20 bg-white dark:bg-form-input">
                  ...
                  >
                    ...
                    {roles.map((role, index) => (
                      <option
                        selected={role.roleID === formData.roleID}
                        key={index}
                        value={role.roleID}
                      >
                        {role.name}
      ...
      >
        <g opacity="0.8">
          ...
          ></path>
              ...
              <button
                type={`submit`}
                className="self-center h-12 font-medium rounded bg-primary text-gray"
              >
                Kirim
              </button>
            </div>
...
export async function getServerSideProps(context) {
  const user = await axios
    .get(`${process.env.NEXT_PUBLIC_API_URL}/user/getAll`, {
      headers: {
        "x-access-token": context.req.cookies.token,
      },
    })
    .then((res) => res.data.data.filter((item) => item.userID === context.params.id));
  if (!user[0]) {
    return {
  ...
  //   .get(`${process.env.NEXT_PUBLIC_API_URL}/company/getAll`, {
  //     headers: {
  //       "x-access-token": context.req.cookies.token,
  //     },
  //   })
